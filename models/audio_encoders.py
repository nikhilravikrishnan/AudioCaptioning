...
def get_audio_feature_vector(model: nn.Module, img_tensors: torch.Tensor, layer_dict={"avgpool":"features"}):
    """
...
def get_cnn14_feature_vector(model: nn.Module, img_tensors: torch.Tensor, layer_dict={"avgpool":"features"}):
    """
    Get feature vectors for the given audio (passed in as a spectrogram image) by returning the output
    of the selected model at an intermediate layer.
